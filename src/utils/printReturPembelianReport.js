...
export const exportReturPembelianToExcel = (reportData, filters = {}) => {
  ...
  // Header information
  csvContent.push(["LAPORAN RETUR PEMBELIAN"]);
  ...
  const filename = `Laporan_Retur_Pembelian_${currentDate}.csv`;
...
export const exportReturPembelianToExcelAdvanced = (
  reportData,
  filters = {},
  XLSX = null
) => {
  try {
    ...
    if (!XLSX && typeof window.XLSX === "undefined") {
      ...
      return exportReturPembelianToExcel(reportData, filters);
    }
    ...
    // Header information
    wsData.push(["LAPORAN RETUR PEMBELIAN"]);
    ...
    xlsxLib.utils.book_append_sheet(wb, ws, "Laporan Retur Pembelian");
...
export const printReturPembelianReport = (reportData, filters = {}) => {
  // Calculate totals
  const totalCarton =
    reportData?.reduce((sum, item) => sum + (item.carton_quantity || 0), 0) ||
    0;
  ...
  if (previewWindow) {
    ...
    const previewHtml = `
      ...
      <head>
        <meta charset="UTF-8">
        <title>Preview - LAPORAN RETUR PEMBELIAN</title>
        <style>
          body {
            margin: 0;
      ...
      <body>
        <div class="preview-container">
          <div class="preview-header">
            <h1 class="preview-title">Preview - Laporan Retur Pembelian</h1>
...
export const downloadReturPembelianReportAsPDF = async (
  ...
  try {
    ...
    printReturPembelianReport(reportData, filters);
  } catch (error) {
